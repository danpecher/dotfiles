...
theme_dir=/tmp/home/.config/tmux/plugins/tmux-gruvbox
mkdir -p "$theme_dir"
printf '%s\n' '#!/bin/sh' 'tmux set-option -g @gruvbox-validation loaded' \
    > "$theme_dir/gruvbox-tpm.tmux"
chmod +x "$theme_dir/gruvbox-tpm.tmux"
HOME=/tmp/home tmux -L dotfiles-validation -f /src/dot_tmux.conf new-session -d -s validation
test "$(HOME=/tmp/home tmux -L dotfiles-validation show-option -gqv @gruvbox-validation)" = loaded
HOME=/tmp/home tmux -L dotfiles-validation kill-server
